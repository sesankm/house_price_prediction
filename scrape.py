...
for city in cities:
    for i in range(1, 40):
        if(i == 1):
            resp = requests.get("https://homefinder.com/for-sale/TX/{}".format(city))
        else:
            resp = requests.get("https://homefinder.com/for-sale/TX/{}?page={}".format(city, i))
        ...
        data += [i[:2] + j + [city] for i,j in zip(prices,attributes) if "Sale" in i and len(i) == 4]

df = pd.DataFrame(data)
df.columns = ["Price", "Type", "Bedrooms", "Bathrooms", "Square Feet", "City"]
